...
async def generate_list_of_names(callback: CallbackQuery, state: FSMContext):
    await callback.answer()

    data = await state.get_data()
    keyboard_buttons: List[List[InlineKeyboardButton]] = data.get(
        'keyboard_buttons', [])
    list_name_for_report = data.get('list_name_for_report', [])
    callback_increment = data.get('callback_increment', 0)
    list_of_all_names_for_report = data.get('list_of_all_names_for_report', [])

    int_number = int(callback.data)

    if int_number in list(range(len(list_of_all_names_for_report))):
        item_text = keyboard_buttons[int_number][0].text
        item_name = item_text.replace('✅', '')

        if item_name not in list_name_for_report:
            list_name_for_report.append(item_name)

            keyboard_buttons[int_number][0] = InlineKeyboardButton(
                text=f"{item_name}✅",
                callback_data=str(int_number + callback_increment)
            )

    elif int_number in [callback_increment + n for n in
                        range(len(list_of_all_names_for_report))]:
        get_index_back = int_number - callback_increment

        if get_index_back < len(keyboard_buttons):
            item_text = keyboard_buttons[get_index_back][0].text
            item_name = item_text.replace('✅', '')

            if item_name in list_name_for_report:
                list_name_for_report.remove(item_name)

                keyboard_buttons[get_index_back][0] = InlineKeyboardButton(
                    text=f"{item_name}",
                    callback_data=str(get_index_back)
                )

    keyboard = InlineKeyboardMarkup(inline_keyboard=keyboard_buttons)

    await callback.message.edit_text(
        text="Выберите кого выводить в отчете:",
        reply_markup=keyboard
    )
...
async def end_configure_report_of_balances(callback: CallbackQuery,
                                           state: FSMContext):
    await callback.answer()
    ...
    if callback.data == 'Закрыть':
        ...
        asyncio.create_task(
            delete_message_after_delay(callback.bot, context_for_job, 3)
        )
